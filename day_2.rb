# Format input to be array of arrays
# Detemine largest value in row
# Determine smallest value in row
# Calculate largest - smallest
# Add this number to an accumulator
# Do these four steps for every row
# Return accumulator

input = "626  2424	2593	139	2136 163	1689	367	2235	125	2365	924	135	2583	1425	2502
...
line_broken_input = input.split(/\n/);

line_broken_input.each_with_index do |line, index|
  line_broken_input.delete_at(index) if line == "\n"

  new_input << line.split(/\t|\s+/)
end
...
#   line.each do |number|
#     number = number.to_i
#     lowest_number = number if lowest_number == nil
#     highest_number = number if highest_number == nil
#     highest_number = number if ((number > highest_number) && (number % lowest_number == 0))
#     lowest_number = number if ((number < lowest_number) && (highest_number % number == 0))
#   end
#   final_tally += (highest_number - lowest_number)
# end

new_input.each do |line|
  line.each do |number|
    number = number.to_i
    line.each do |comparison_number|
      comparison_number = comparison_number.to_i
      if number != comparison_number
        if number % comparison_number == 0
          "#{number} and #{comparison_number} are DIVISIBLE"
          final_tally += (number / comparison_number)
        end
      end
    end
  end
